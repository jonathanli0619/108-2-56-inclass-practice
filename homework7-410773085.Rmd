...
```{r ans21}
Mask[[1]][[1]]
as.integer(str_extract(Mask[[1]][[1]],"[0-9]{9}"))->select1

select1%%2

funcrionZ<-function(n){
  if(as.logical(n %% 2)){
     aans<- 1
  } else {
aans<- 0
...
funcrionZ(select1)->`1st_odd_or_even`
# `1st_odd_or_even`
```
...
`odd_or_even`<-function(n){
  as.integer(str_extract(n,"[0-9]{9}"))->pick1 
  
    if(as.logical(pick1 %% 2)){
     aans<- 1
  } else {
aans<- 0
  }
...
chart[[odd_or_even(Mask$身分證字號[[1]])+1]]->check1

functionP<-function(n){
  if(wday(today())%in%n){
     aanss<- today()
  } else {
aanss<- today()+1
...
functionP(check1)->`1st_next_date`
`1st_next_date`
# 1st_next_date
```
...
```{r ans24}
next_date<-function(n){
  chart[[odd_or_even(n)+1]]->check1
  
  if(wday(today())%in%check1){
     aanss<- today()
  } else {
aanss<- today()+1
  }
...
```{r ans31}
get_courses<-function(n,m){
  as.integer(n)->n1
c1<-unique(courseStructure$科目名稱[courseStructure$學年==as.character(n1)&str_detect(courseStructure$應修系級,paste0(m,1))])
 c2<-unique(courseStructure$科目名稱[courseStructure$學年==as.character(n1+1)&str_detect(courseStructure$應修系級,paste0(m,2))]) 
c3<-unique(courseStructure$科目名稱[courseStructure$學年==as.character(n1+2)&str_detect(courseStructure$應修系級,paste0(m,3))])
c4<-unique(courseStructure$科目名稱[courseStructure$學年==as.character(n1+3)&str_detect(courseStructure$應修系級,paste0(m,4))])
  app<-list(
  year1=c1,
  year2=c2,
  year3=c3,
