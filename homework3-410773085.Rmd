...
```{r ans13}
pick_most <- max(table_countsByArticle)
table(badEmployers$law[max(table_countsByArticle)])
...
table_countsByViolatedArticle
max(table_countsByViolatedArticle)
badEmployers$lawContent[max(table_countsByViolatedArticle)]
...
logical儲存的本質是甚麼?
```{r ans23}
pick_chineseabove12 <- students10$國文 >= 12
pick_engmathabove26 <- students10$英文+students10$數學 >= 26
students10$姓名[pick_chineseabove12]
students10$姓名[pick_engmathabove26]
